...
function extractKeywordsForSearch(text: string): string {
  ...
  if (
    /responsabilité|limitation.*responsabilité|dommages.*indirects/.test(
      cleaned,
    )
  ) {
  ...
  if (/non.?concurrence|concurrence|interdiction.*activité/.test(cleaned)) {
    ...
    return ["clause non-concurrence", "validité", durationHint, scope, post]
      .filter(Boolean)
      .join(" ")
      .trim();
  }
...
export async function searchLegifranceViaProxy(
  query: string,
  limit = 3,
): Promise<CanLiiCase[]> {
  ...
  console.log("➡️ Proxy search Légifrance (JURI):", {
  ...
  const pickResultArray = (root: any): any[] => {
      ...
      for (const [k, v] of Object.entries(root)) {
        ...
        if (v && typeof v === "object") {
          const vv: any =
            (v as any).results ||
            (v as any).resultats ||
            (v as any).resultsListe ||
            (v as any).items;
          if (Array.isArray(vv) && vv.length)
            candidates.push({ label: `sub.${String(k)}`, arr: vv });
        }
      }
    }
    ...
    if (top)
      console.log(
        "🧭 Légifrance results array picked:",
        top.label,
        "len=",
        top.arr.length,
      );
  ...
  if (!items.length) {
    console.warn(
      "⚠️ Aucune liste de résultats détectée dans la réponse Légifrance.",
    );
  }
  ...
  for (const it of pool) {
    ...
    const numero =
      (it as any).numeroDecision || (it as any).cid || (it as any).nor || "";
    const dateStr =
      (it as any).dateDecision ||
      (it as any).datePrononcee ||
      (it as any).datePublication ||
      "";
    const court =
      (it as any).juridiction ||
      (it as any).juridictionJudiciaire ||
      (it as any).formation ||
      (it as any).chambre ||
      "Juridiction";
    const titre =
    ...
    const tParsed =
      typeof dateStr === "string" && dateStr ? Date.parse(dateStr) : NaN;
    ...
    console.log(`📋 Décision mappée ${out.length + 1}:`, {
      title: mappedCase.title,
      court: mappedCase.court,
      year: mappedCase.year,
      url: mappedCase.url,
    });
    out.push(mappedCase);
  }
...
export async function consultLegifranceJuriText(
  juriId: string,
): Promise<any | null> {
  try {
    const res = await withTimeout(
...
function extractDateFromTitleDate(title: string): {
  dateIso?: string;
  year?: number;
} {
  if (!title) return {};
...
export async function top3LegifranceGlobal(
  query: string,
  typeRecherche: "EXACTE" | "UN_DES_MOTS" | "TOUS_LES_MOTS" = "UN_DES_MOTS",
): Promise<CanLiiCase[]> {
  const today = new Date().toISOString().slice(0, 10);
  ...
  try {
      ...
      const filtered = alt
        .map((c) => {
          ...
          if (typeof c.date === "string" && !Number.isNaN(Date.parse(c.date))) {
            ...
            if (t?.year) {
              year = t.year;
              ts = t.dateIso ? Date.parse(t.dateIso) : Date.UTC(year, 0, 1);
            }
          }
          return { c, year, ts };
        })
    ...
    const out: CanLiiCase[] = arr
      .map((r, i) => {
        ...
        let ts = NaN;
        if (typeof r?.dateDecision === "string" && r.dateDecision)
          ts = Date.parse(r.dateDecision);
        if (
          Number.isNaN(ts) &&
          typeof r?.datePublication === "string" &&
          r.datePublication
        )
          ts = Date.parse(r.datePublication);
        let year = Number.isNaN(ts) ? 0 : new Date(ts).getUTCFullYear();
        if (!year || year < 2020) {
          const t = extractDateFromTitleDate(titre);
          if (t?.year && t.year >= 2020) {
            year = t.year;
            if (Number.isNaN(ts) && t.dateIso) ts = Date.parse(t.dateIso);
          }
        }
        if (!year || year < 2020 || year > yNow) return null;
        const dateOut = !Number.isNaN(ts)
          ? new Date(ts).toISOString().slice(0, 10)
          : typeof r?.dateDecision === "string"
            ? r.dateDecision
            : undefined;
        return {
          id: `legi_${id || i}`,
