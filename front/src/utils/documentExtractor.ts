...
const PDF_SERVER_URL =  `${import.meta.env.VITE_BACKEND_PYTHON_URL}/extract-pdf-text` 
const MAX_RETRIES = 2;
...
export async function extractDocumentContent(file: File): Promise<ExtractedContent> {
  ...
  try {
    ...
    console.warn('⚠️ Serveur local indisponible, basculement vers PDF.js:', error);
  }
  ...
  try {
    ...
    throw new Error('Impossible d\'extraire le texte du document. Pour les PDF scannés, assurez-vous que le serveur Python est démarré.');
  }
}
 ...
 */
async function extractViaServer(file: File, retries = MAX_RETRIES): Promise<ExtractedContent> {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      console.log(`✅ Extraction serveur réussie: ${data.text.length} caractères`);
      ...
      if (error.name === 'AbortError') {
        console.log(`⏹️ Tentative ${attempt}/${retries} annulée (nouveau fichier chargé)`);
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      try {
        ...
        console.log(`📄 Page ${pageNum}/${totalPages} extraite (${pageText.length} caractères)`);
      } catch (pageError) {
    ...
    console.log(`🧹 Nettoyage du texte extrait (${rawText.length} caractères bruts)...`);
    ...
    console.log(`✅ Texte nettoyé: ${rawText.length} → ${formattedText.length} caractères`);
    console.log(`📄 Aperçu du texte nettoyé: ${formattedText.substring(0, 200)}...`);

    const quality = formattedText.length > 1000 ? 'high' :
      formattedText.length > 100 ? 'medium' : 'low';

    console.log(`✅ Extraction PDF.js terminée: ${formattedText.length} caractères`);
 ...
 */
function applyLightSanitizeIfEnabled(text: string, source: 'server' | 'pdfjs'): string {
  ...
  try {
    ...
    if (report.aborted) {
      console.warn(`⚠️ LightSanitize abort (ratio=${(report.ratioRemovedNonWhitespace * 100).toFixed(2)}%) → texte original conservé.`);
      return text;
    }
    return sanitized;
  } catch (e) {
