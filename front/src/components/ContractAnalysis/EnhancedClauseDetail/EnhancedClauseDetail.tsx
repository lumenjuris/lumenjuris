...
const ChatUI = React.lazy(
  () => import("../../ContractAnalysis/EnhancedClauseDetail/ChatUI"),
);
...
export const EnhancedClauseDetail: React.FC<Props> = ({
  clause,
  context,
  onClose,
  recommendationIndex,
  ...
  const ai = useAIStore((s) =>
    clause ? s.map[getClauseAIKey(clause.id, clauseAiModel)] : undefined,
  );
  const fetchAI = useAIStore((s) => s.fetch);
  useEffect(() => {
    if (clause && !ai) fetchAI(clause, clauseAiModel);
  }, [clause, ai, fetchAI, clauseAiModel]);
  ...
  const [automaticDecisions, setAutomaticDecisions] = useState<
    JurisprudenceCase[]
  >([]);
  useRun(
    clause,
    ...
    setAutomaticDecisions,
    jurisprudenceCache,
  );

  const longText = useMemo(() => clause.content.trim(), [clause]);
  const shortText = useMemo(() => {
    if (!expanded && longText.length > 800)
      return longText.slice(0, 750) + "...";
    return longText;
  }, [expanded, longText]);
        ...
        <Suspense
          fallback={
            <div className="p-4 text-sm text-slate-500">
              Chargement du module de questions…
            </div>
          }
        >
          <ChatUI
